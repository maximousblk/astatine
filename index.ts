 ...
 */
const roundTo = (num: number, int: number) =>
    Math.round(num / int / 1000) * int;
    ...
    linear: (x: number) =>
        Math.floor(
            config.initial_emit_amount -
                (x * config.time_interval * config.initial_emit_amount) /
                    config.emission_period
        ),
    exponential: (x: number) =>
        Math.floor(
            config.initial_emit_amount *
                Math.E ** (-config.decay_const * x * config.time_interval)
        ),
};

const dist_curve: string = isNaN(config.decay_const) ? "linear" : "exponential";
const dist_total: number = sigma(
    0,
    config.emission_period / config.time_interval,
    dist[dist_curve]
);
    ...
    for (let i = 0; i < addresses.length; i++) {
        const tags = {
            ...
            Input: JSON.stringify({
                ...
                qty: addresses[i].weight
                    ? Math.floor((amount * addresses[i].weight) / weightTotal)
...
async function runDistribution() {
    ...
    if (expend > 0) {
        ...
        let transactions = await primeCannon(
            expend,
            config.token_allocations,
            time
        );
